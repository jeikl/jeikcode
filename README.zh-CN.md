...
AtomCode 专门针对弱模型 / 廉价模型（DeepSeek V3、GLM-5、Qwen-Plus）做了优化：

- **紧凑的系统提示** —— 约 1.5K tokens，规则放在末尾（近因效应）
- **不预读源文件** —— 由模型自己按需读取，避免注意力被稀释
- **Token 预算感知的会话窗口** —— 冷热区分，工具结果自动压缩
- **周期性系统提醒** —— 每隔几步提醒模型当前目标，防止跑偏
- **专用 JSON 修复** —— 处理弱模型产出的畸形 function-call 参数
- **编辑上下文回传** —— 每次编辑后回传周围几行代码帮助模型定位
