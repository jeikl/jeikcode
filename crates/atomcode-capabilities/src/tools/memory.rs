...
impl Tool for MemoryTool {
    ...
    async fn execute(&self, args: &str, ctx: &ToolContext) -> ToolResult {
        ...
        match a.action.as_str() {
            "remember" => {
                let content = match a.content.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
                    Some(c) => c,
            ...
            "forget" => {
                let keyword = match a.keyword.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
                    Some(k) => k,
            ...
            other => err(format!("memory: unknown action '{other}'. Use remember | forget | list.")),
        }
    }
}
...
mod tests {
    ...
    async fn remember_writes_project_entry() {
        ...
        let r = MemoryTool.execute(r#"{"action":"remember","content":"uses tabs"}"#, &ctx(tmp.path())).await;
        assert!(!r.is_error, "{}", r.content);
        assert!(crate::memory::MemoryStore::project(tmp.path()).load().iter().any(|e| e == "uses tabs"));
    }

    #[tokio::test]
    async fn remember_dedup_reports_skip() {
        let tmp = tempfile::tempdir().unwrap();
        MemoryTool.execute(r#"{"action":"remember","content":"x"}"#, &ctx(tmp.path())).await;
        let r = MemoryTool.execute(r#"{"action":"remember","content":"x"}"#, &ctx(tmp.path())).await;
        assert!(!r.is_error);
    ...
    async fn remember_missing_content_errors_not_panics() {
        let tmp = tempfile::tempdir().unwrap();
        let r = MemoryTool.execute(r#"{"action":"remember"}"#, &ctx(tmp.path())).await;
        assert!(r.is_error);
    }
    ...
    async fn forget_removes_matching() {
        let tmp = tempfile::tempdir().unwrap();
        MemoryTool.execute(r#"{"action":"remember","content":"delete me please"}"#, &ctx(tmp.path())).await;
    ...
    fn risk_is_safe_by_default_and_risky_under_approval_env() {
        ...
        assert!(matches!(MemoryTool.risk(r#"{"action":"remember","content":"x"}"#), RiskLevel::Safe));
        // 开审批 → remember Risky, list 仍 Safe
        std::env::set_var("ATOMCODE_MEMORY_APPROVAL", "1");
        assert!(matches!(MemoryTool.risk(r#"{"action":"remember","content":"x"}"#), RiskLevel::Risky));
        assert!(matches!(MemoryTool.risk(r#"{"action":"list"}"#), RiskLevel::Safe));
        std::env::remove_var("ATOMCODE_MEMORY_APPROVAL");
    }
}
