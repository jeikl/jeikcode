...
pub fn assemble(
    ...
    // CC external hooks (PreToolUse gate), in the permission-gate cluster RIGHT BEFORE
    // approval. This placement is load-bearing and matches Claude Code, where a
    // PreToolUse hook runs as part of the permission flow: its `updatedInput` rewrite
    // lands BEFORE approval (so the user approves the exact bytes that run), its `allow`
    // short-circuits the chain to bypass the approval prompt, and its `deny` blocks
    // outright. Registered only when hooks exist (else zero middleware overhead).
    if let Some(cc) = &parts.cc_external_hooks {
        builder = builder.middleware(cc.clone());
    }
    let mut builder = builder
        // Approval AFTER the CC PreToolUse gate above — the user approves the exact bytes
        // that run (any hook `updatedInput` rewrite has already applied).
