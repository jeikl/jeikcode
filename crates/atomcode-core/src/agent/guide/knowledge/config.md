...
[plugin]                  # 插件
auto_install_default_skills = true   # 首次启动自动安装默认技能包
auto_update_marketplaces = true      # 升级后自动更新市场
```
...
- **环境变量替代**：在 CI 环境中，可通过 shell 脚本或 CI secrets 机制动态写入配置文件，避免硬编码 API Key
