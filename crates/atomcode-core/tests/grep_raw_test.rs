...
async fn test_raw_rg_from_rust() {
    ...
    eprintln!("FIRST 3 LINES: {:?}", stdout.lines().take(3).collect::<Vec<_>>());

    assert!(!stdout.is_empty(), "rg should find jwt in backend/src. stderr={}", stderr);
    assert!(stdout.contains("JwtService") || stdout.contains("jwt"),
...
async fn test_raw_rg_regex_or() {
    ...
    assert!(!stdout.is_empty(), "rg with | OR should find results. stderr={}", stderr);
}
...
async fn test_raw_rg_relative_path() {
    ...
    assert!(!stdout.is_empty(), "rg with relative path + current_dir should work");
}
...
async fn test_grep_tool_on_devpress() {
    ...
    eprintln!("TEST1 (abs path): success={} output_lines={}", result.success, result.output.lines().count());
    assert!(result.success, "grep jwt with absolute path failed. Output: {}", result.output);
    ...
    let result = tool.execute(&args, &ctx).await.unwrap();
    eprintln!("TEST2 (rel path): success={} output_lines={}", result.success, result.output.lines().count());
    assert!(result.success, "grep jwt with relative path failed. Output: {}", result.output);
    ...
    let result = tool.execute(&args, &ctx).await.unwrap();
    eprintln!("TEST3 (regex OR): success={} output_lines={}", result.success, result.output.lines().count());
    assert!(result.success, "grep jwt|token failed. Output: {}", result.output);
    ...
    let result = tool.execute(&args, &ctx).await.unwrap();
    eprintln!("TEST4 (search in frontend): success={} output_lines={}", result.success, result.output.lines().count());
    assert!(result.success, "grep 'search' in frontend/src failed. Output: {}", result.output);
    ...
    let result = tool.execute(&args, &ctx).await.unwrap();
    eprintln!("TEST5 (CJK|ASCII): success={} output_lines={}", result.success, result.output.lines().count());
    assert!(result.success, "grep '搜索|search|keyword' failed. Output: {}", result.output);
}
