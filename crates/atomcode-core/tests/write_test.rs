...
async fn test_write_new_file() {
    ...
    assert!(result.output.contains("Created new file"), "output: {}", result.output);
...
async fn test_write_overwrite_existing() {
    ...
    let result = tool.execute(&args, &ctx).await.unwrap();
    assert!(result.success, "should succeed: {}", result.output);
    assert!(result.output.contains("Overwrote"), "should say Overwrote: {}", result.output);
    assert!(result.output.contains("was 3 lines"), "should mention old line count: {}", result.output);

    // Verify content is new
    let content = std::fs::read_to_string(&file_path).unwrap();
    assert!(content.contains("new_main"), "file should contain new content");
    assert!(!content.contains("old"), "file should not contain old content");
}
...
async fn test_write_overwrite_warns_on_shrink() {
    ...
    let old_content = (0..30).map(|i| format!("fn func_{}() {{}}", i)).collect::<Vec<_>>().join("\n");
    ...
    assert!(result.output.contains("WARNING"), "should warn on significant shrink: {}", result.output);
    assert!(result.output.contains("shrank"), "should mention shrank: {}", result.output);
}
...
async fn test_write_overwrite_no_warn_on_growth() {
    ...
    let new_content = (0..30).map(|i| format!("fn func_{}() {{}}", i)).collect::<Vec<_>>().join("\n");
    let args = json!({
        "file_path": file_path,
        "content": new_content
    }).to_string();
    ...
    let result = tool.execute(&args, &ctx).await.unwrap();
    assert!(result.success);
    assert!(!result.output.contains("WARNING"), "should NOT warn on growth: {}", result.output);
}
...
async fn test_write_creates_parent_dirs() {
    ...
    let file_path = dir.path().join("deep/nested/dir/file.rs").to_string_lossy().to_string();
    let args = json!({
        "file_path": file_path,
        "content": "fn nested() {}\n"
    }).to_string();
    ...
    let result = tool.execute(&args, &ctx).await.unwrap();
    assert!(result.success, "should create parent dirs: {}", result.output);
    assert!(std::path::Path::new(&file_path).exists());
}
