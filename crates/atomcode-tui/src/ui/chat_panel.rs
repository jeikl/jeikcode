...
pub fn render(
    ...
    if cache_key != *render_cache_msg_count {
        ...
        let total_tool_results = conversation
            .messages
            .iter()
            .filter(|m| {
                matches!(
        ...
        let results_in_current_turn = conversation.messages[last_user_idx..]
            .iter()
            .filter(|m| {
                matches!(
                    &m.content,
        ...
        for msg in &conversation.messages {
            match &msg.content {
                ...
                MessageContent::ToolResult(r) => {
                    completed_call_ids.insert(r.call_id.as_str());
                }
            ...
            match &msg.content {
                ...
                MessageContent::AssistantWithToolCalls { text, tool_calls } => {
                    ...
                    while ci < all_calls.len() {
                        ...
                        let batch_completed =
                            completed_call_ids.contains(all_calls[ci].id.as_str());
                        while ci < all_calls.len()
                        ...
                        if batch_count >= 2 && ci <= fold_before && batch_completed {
                            let details: Vec<String> = all_calls[batch_start..ci]
                                .iter()
                                .map(|c| extract_tool_detail_short(&c.name, &c.arguments))
                                .collect();
                            render_batch_tool_calls(
                                render_cache,
                                batch_tool,
                                batch_count,
                                &details,
                ...
                MessageContent::ToolResult(_) | MessageContent::ToolResultRef(_) => {
                    ...
                    let _is_read = call_id_to_tool.get(call_id) == Some(&"read_file");
                    ...
                    let tool_name = call_id_to_tool.get(call_id).map(|s| &**s).unwrap_or("");
                    {
                        ...
                        if batch_count >= 2 && is_early_result {
                            ...
                            let details: Vec<String> = (i..batch_end)
                                .map(|idx| {
    ...
    if !pending_appends.is_empty() {
        ...
        for queued in pending_appends {
            ...
            dynamic.push(Line::from(Span::styled(
                format!(
                    "{}\u{21b5} queued \u{00b7} will send after this turn",
                    INDENT
                ),
                Style::default()
                    .fg(theme::text_muted())
    ...
    let _ = step_count;
    let turn_active =
        last_turn_duration.is_none() && matches!(mode, AppMode::Streaming | AppMode::ToolExecuting);
    if turn_active && area.height >= 2 {
        ...
        let step_prefix = if step_count > 0 {
            format!("[turn {}] ", step_count)
        } else {
            String::new()
        };
        let has_tokens = conversation
            .stream_buffer
            .as_ref()
...
fn render_tool_call(lines: &mut Vec<Line<'static>>, call: &ToolCall, icon_override: Option<&str>) {
    ...
    let icon_color = if icon_override.is_some() {
        theme::accent()
    } else {
...
fn render_tool_result(
    ...
    let summary = if output.is_empty() || output.trim().is_empty() {
        ...
        || output.starts_with("[Loop")
        || output.starts_with("[SKIPPED")
    {
        output.lines().next().unwrap_or("").to_string()
    } else {
        ...
        let first_short = if first.chars().count() > 50 {
            format!(
                "{}...",
                first.chars().take(text_limit.min(50)).collect::<String>()
            )
        } else {
            first.to_string()
        };
    ...
    let result_text_style = Style::default()
        .fg(theme::text_muted())
            ...
            if output_lines.len() > 1 + max_expanded {
                lines.push(Line::from(vec![
                    ...
                    Span::styled(
                        format!(
                            "    ... ({} more lines)",
                            output_lines.len() - 1 - max_expanded
                        ),
                        Style::default().fg(theme::text_muted()),
                    ),
                ]));
...
fn extract_tool_detail_short(tool_name: &str, args_json: &str) -> String {
    ...
    match tool_name {
        "read_file" | "write_file" | "create_file" | "edit_file" => args
            .get("file_path")
...
fn format_tool_detail(tool_name: &str, args_json: &str) -> String {
    ...
    match tool_name {
        ...
        "bash" => {
            ...
            if first.chars().count() > 55 {
                first.chars().take(52).collect::<String>() + "..."
            } else {
                first.to_string()
            }
        }
