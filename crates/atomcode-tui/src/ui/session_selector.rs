...
pub fn render(
    ...
    let filtered: Vec<(usize, &SessionMeta)> = sessions
        .iter()
        .enumerate()
    ...
    let search_bg = if search_is_selected {
        theme::brand_bg()
    } else {
        theme::bg_surface()
    };
    let search_fg = if search_is_selected {
        theme::text_primary()
    } else {
    ...
    let search_indicator = if search_is_selected {
        " \u{25b8} "
    } else {
        "   "
    };
    ...
    if query.is_empty() {
        lines.push(Line::from(vec![
            ...
            Span::styled(
                " Type to search...",
                Style::default().fg(theme::text_muted()).bg(search_bg),
            ),
        ]));
    } else {
