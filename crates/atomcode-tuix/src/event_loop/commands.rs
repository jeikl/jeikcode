    ...
    match cmd {
        ...
        "mcp" => {
            ...
            if sub.eq_ignore_ascii_case("reload") {
                ...
                // The coding runtime owns its tool registry. ReloadCapabilities below
                // replaces the whole prepared capability set, so the TUI has no
                // parallel registry to mutate or count here.
                let removed = 0;
