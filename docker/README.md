...
```bash
docker build -t atomcode-daemon:v4.24.0 -f docker/Dockerfile-Daemon .
```
...
```bash
# 标记镜像
docker tag atomcode-daemon:v4.24.0 swr.cn-north-4.myhuaweicloud.com/gitcode-be/atomcode-daemon:v4.24.0

# 使用 buildx 构建并推送（推荐）
docker buildx build --provenance=false --platform linux/amd64 -t swr.cn-north-4.myhuaweicloud.com/gitcode-be/atomcode-daemon:v4.24.0 --push -f docker/Dockerfile-Daemon .

# 或者先构建再推送
docker build --provenance=false -t swr.cn-north-4.myhuaweicloud.com/gitcode-be/atomcode-daemon:v4.24.0 -f docker/Dockerfile-Daemon .
docker push swr.cn-north-4.myhuaweicloud.com/gitcode-be/atomcode-daemon:v4.24.0
```
