...
| `turn/truncation.rs` | 504 | **输出截断** — 按工具类型截断（bash 保错误行、read 出 skeleton）、>512B 外部化到 result_store |
| `turn/permission.rs` | ~100 | **权限决策** — InteractivePermissionDecider（弹确认框）、AutoBypass/AutoDeny |
| `turn/json_repair.rs` | 439 | **JSON 修复** — 弱模型输出的 JSON 常有语法错误（缺逗号、单引号、未闭合），自动修复 |
