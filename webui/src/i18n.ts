...
const zh = {
  ...
  'chat.detachedActive': '此会话由其他客户端（如 OpenAI API）发起，正在实时同步进度…',
  'chat.detachedPolling': '后台回合进行中… 正在实时同步',
  'chat.detachedWatching': '正在实时同步 API 客户端的回合进度…',
  ...
  'chat.detachedSendBlocked': '此会话正被 API 客户端占用一个回合，WebUI 只能实时同步进度，不能发送消息。请等回合结束或停止后再发。',
...
const en: Record<MsgKey, string> = {
  ...
  'chat.detachedActive': 'This session was started by another client (e.g. OpenAI API). Live progress is being synced…',
  'chat.detachedPolling': 'Background turn in progress… live syncing',
  'chat.detachedWatching': 'Live-syncing progress from the API client turn…',
  ...
  'chat.detachedSendBlocked': 'This session is running a turn owned by an API client; the WebUI can only live-sync progress and cannot send messages. Wait for the turn to finish or stop it first.',
